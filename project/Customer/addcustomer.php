...
<head>
    <title>CS321 Project • Search</title>
    <meta charset="utf-8">
    ...
    <div>
        ...
        <div class="container mt-5">
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                $query = "INSERT INTO Customer (CustomerID, fname, lname, phoneNumber, address) VALUES (?, ?, ?, ?, ?)";
                $stmt = $conn->prepare($query);
                $stmt->bind_param("issss", $customerID, $firstName, $lastName, $phoneNumber, $address);
                $status = $stmt->execute();
                if ($status) {
                    echo "<div class='alert alert-success'>Successfully inserted new customer $firstName $lastName</div>";
                } else {
                    echo "<div class='alert alert-danger'>Failed to insert customer.</div>";
                }
            }
            ?>
        </div>
